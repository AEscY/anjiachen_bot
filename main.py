...
async def cmd_positions(msg: Message):
    ...
    for iid in dlg.MANAGER.all_inst_ids():
        ...
        lines.append(f"  浮动盈亏: {profit_pct:+.2f}%")
    ...
    if len(parts) < 2 or parts[1].lower() != "confirm":
        ...
        for iid in dlg.MANAGER.all_inst_ids():
            ...
            if d and d.position > 0:
                ...
                lines.append(
                    f"  <b>{iid}</b>: {d.position:.6f}\n"
                    f"    均价 {avg:.4f} | 现价 {price:.4f} | 浮盈 {profit_pct:+.2f}%"
                )

        if not has_pos:
            await msg.answer("当前无任何持仓")
            return

        lines.append("")
        lines.append("⚠️ 清仓会：")
        lines.append("  1. 立即以市价卖出所有持仓")
        lines.append("  2. 暂停所有策略（不会自动重启）")
        lines.append("")
        lines.append("确认请输入：<code>/closeall confirm</code>")
        await msg.answer("\n".join(lines), parse_mode="HTML")
        return

    await msg.answer("🔄 正在清仓，请稍候...")
    results = await dlg.MANAGER.close_all_positions()
    lines = ["📋 <b>清仓结果</b>", ""]
    lines.extend(f"  {r}" for r in results)
    lines.append("")
    lines.append("所有策略已暂停。如需继续交易，请进入各币种面板手动启动。")
    await msg.answer("\n".join(lines), parse_mode="HTML")
...
async def setup_menu():
    ...
    await bot.set_my_commands([
        ...
        BotCommand(command="positions", description="持仓总览"),
        ...
        BotCommand(command="closeall", description="一键清仓"),
