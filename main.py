...
from ui.dialogs import get_dialogs, _last_price, MANAGER
...
# 全局 WebSocket 引用，供 UI 层调用 subscribe/unsubscribe
pub_ws: "PublicWS | None" = None


async def on_ticker(inst_id: str, price: float, raw: dict):
    _last_price[inst_id] = price
    if MANAGER:
        await MANAGER.on_ticker(inst_id, price, raw)
...
async def cmd_list(msg: Message):
    ...
    if not MANAGER:
        ...
        return
    lines = [f"• {iid}  {_last_price.get(iid, '-')}" for iid in MANAGER.all_inst_ids()]
...
async def main():
    ...
    # 初始化 Manager
    dlg.MANAGER = StrategyManager()

    # 添加配置中的初始币种
    for iid in WATCHLIST:
        ok, text = await dlg.MANAGER.add_inst(iid)
        logger.info(text)
    ...
    asyncio.create_task(pub_ws.connect(dlg.MANAGER.all_inst_ids()))
    asyncio.create_task(priv_ws.connect())

    # 让 UI 层能通过 manager.middleware_data 拿到 pub_ws（用于订阅/退订）
    dp.workflow_data.update({"ws_public": pub_ws})
    ...
    await alert(
        ...
        f"时间: {now}\n"
        f"币种: {', '.join(dlg.MANAGER.all_inst_ids())}"
    )
