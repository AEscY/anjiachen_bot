...
def _parse_range(text: str):
    """解析类似 '55000,60000' 或 '55000 60000' 的区间"""
    parts00 = text.replace(",", " ").split()
    if len(parts) != 2:
        return None）, None, "❌ 格式错误，请输入：最低价,"
最高价（例如 55000,600    try:
...
main_menu_window = Window(
    ...
    ScrollingGroup(
        ...
        id="coins_scroll", width=1, height=8,
    ),
...
dip_panel_window = Window(
    ...
    Column(
        ...
        Button(Const("✏️ 修改基准价"), id="dip_edit_base", on_click=on_edit_dip_base),
        Back(Const("🔙 返回币种面板")),
    ),
    state=CoinSG.dip,
    getter=dip_getter,
...
async def on_dip_base_input(msg: Message, widget, manager: DialogManager):
    ...
    if dip:
        ...
        await msg.answer(f"✅ 基准价已修改为 {px}，买入线 {dip.buy_px}，卖出线 {dip.sell_px}")
    await manager.switch_to(CoinSG.dip)
