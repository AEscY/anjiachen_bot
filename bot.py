...
class MarketStateEngine:
    ...
    def detect_state(tech_data, price_history, volatility_history):
        if tech_data is None or len(price_history) < 20:
            return "neutral", 0.5
        volatility = tech_data.get('atr', 0) / tech_data.get('bb_middle', 1) if tech_data.get('bb_middle', 0) > 0 else 0.0
...
class MultiAgentSystem:
    ...
    async def risk_assess(portfolio, market_state):
        ...
        if portfolio.get('consecutive_losses', 0) >= 3:
            risk_score += 0.2
            reasons.append("连续亏损")
        if portfolio.get('drawdown', 0) > 0.12:
            risk_score += 0.25
            reasons.append("回撤超过12%")
...
class FrontierEngine:
    ...
    def meta_rl_score(price_history, win_rate_history, sharpe_history, n=30):
        ...
        return_std = np.std(recent_returns) if len(recent_returns)>1 else 0.0
...
class QuantBot:
    def __init__(self, exchange):
        ...
        self._trailing_high = {}
        self.entry_details = {}
        # FIFO position ledger: only these lots are managed by the bot.
        self.position_lots = {}
        ...
        # 每日重置标记
        self._last_reset_date = datetime.now(CST).date().isoformat()
    ...
    def _append_position_lot(self, sym, amount, price, cost, fee=0.0, currency=''):
        self._position_lots_for(sym).append({
            ...
            'fee': float(fee or 0), 'fee_currency': currency or '', 'time': time.time()
        })
    ...
    def _consume_position_lots(self, sym, amount, exit_price, exit_revenue, sell_fee=0.0):
        ...
        if remaining > 1e-8:
            raise ValueError(f'{sym} 卖出数量超过机器人账本数量: remaining={remaining}')
    ...
    async def _can_allocate(self, additional_usdt):
        balance = self._account_balance or (self._cached_usdt_free + sum(
            self._cached_balances.get(sym.split('/')[0], 0) * ((self.ws.get_ticker(sym) or {}).get('last') or 0)
            for sym in self.symbols))
        if balance <= 0:
            return False
    ...
    async def _round_amount_by_precision(self, symbol: str, amount: float) -> float:
        try:
            ...
            logger.warning(f"数量精度处理失败 {symbol}: {e}")
            return 0.0
    ...
    async def load_and_init(self):
        ...
        if state:
            ...
            self.entry_details = state.get('entry_details', self.entry_details)
            self._daily_start_equity = float(state.get('daily_start_equity', 0) or 0)
            self._today_loss_usdt = float(state.get('today_loss_usdt', 0) or 0)
            ...
            try:
                ...
                for sym in self.symbols:
                    try:
                        ...
                        volatility = tech.get('atr', 0) / tech.get('bb_middle', 1) if tech.get('bb_middle', 0) > 0 else 0.0
    ...
    async def _should_open_position(self, sym, p, tech, funding, fg, usdt_free):
        ...
        ema20 = None
        try:
            ema20 = tech.get('bb_middle') if tech else None
        except:
            pass
        ...
        if multi.get('1m') is None or multi.get('5m') is None or multi.get('15m') is None:
            cross_score = 50.0
            details.append('Cross数据不足-中性')
    ...
    async def _adjust_tp_sl_by_volatility(self, sym):
        tech = await self._get_cached_tech(sym, self.timeframe, 50)
        tp = float(self._get_coin_param(sym, 'tp_pct', self.tp_pct))
        sl = float(self._get_coin_param(sym, 'sl_pct', self.sl_pct))
        if tech and tech.get('bb_middle', 0) > 0:
            vol = max(0.0, float(tech.get('atr', 0) or 0) / float(tech['bb_middle']))
            factor = max(0.75, min(1.75, 1.0 + vol * 8.0))
            tp *= factor
            sl *= max(0.85, min(1.35, factor))
        tp = max(self.breakeven_pct, min(0.05, tp))
        ...
        while self.is_running:
            try:
                if not await self._check_risk_limits():
                    ...
                    continue
                if time.time() - self._last_btc_check_time >= 60:
                    self._last_btc_check_time = time.time()
                    btc = self.ws.get_ticker('BTC/USDT')
                    self._btc_safe_flag = True if not btc else (float(btc.get('percentage') or 0) / 100.0 > -0.04)
                await self._refresh_balance_cache(force=True)
                equity = self._cached_usdt_free
                for sym in self.symbols:
                    ticker = self.ws.get_ticker(sym)
                    if ticker:
                        equity += self._cached_balances.get(sym.split('/')[0], 0) * float(ticker.get('last') or 0)
                if equity > 0:
                    ...
                    if not self._drawdown_safe_flag:
                        self._is_paused = True
                        await self._alert(f'账户回撤 {drawdown*100:.2f}% 达到上限，自动交易暂停', 'critical')
    ...
    async def panic_sell_all(self):
        """紧急全平：只平机器人账本持仓，绝不出售账户里未被机器人管理的资产。"""
        self._is_paused = True
        for sym in self.symbols:
            try:
                ...
                if rounded_amount > 0:
                    order = await self.exchange.create_market_sell_order(sym, rounded_amount)
                    if order:
                        ...
                        try:
                            ...
                            logger.error(f"🚨 Panic Sell 后 {sym} 账本不一致，保持暂停")
    ...
    async def _sync_positions(self):
        """同步时只接受可解释的余额差异；不再用当前价格伪造历史入场价。"""
        try:
            bal = await self.exchange.fetch_balance()
            mismatches = []
            for sym in self.symbols:
                coin = sym.split('/')[0]
                exchange_amount = float((bal.get(coin) or {}).get('free', 0) or 0)
                bot_amount = self._bot_position_amount(sym)
                tolerance = max(1e-8, exchange_amount * 0.002)
                if abs(exchange_amount - bot_amount) > tolerance:
                    mismatches.append((sym, bot_amount, exchange_amount))
            if mismatches:
                self._is_paused = True
                logger.error(f"🚨 持仓账本不一致: {mismatches}")
                await self._alert("持仓账本与交易所余额不一致，自动交易已暂停，请人工核对", "critical")
                return False
            await self._save_runtime_state()
            logger.info("✅ 持仓账本与交易所余额一致")
            return True
        except Exception as e:
            self._is_paused = True
    ...
    async def _open_delta_neutral(self, symbol, funding_rate, amount_usdt=None):
        """只有在实现现货+永续双腿对冲前才允许开启；当前系统明确拒绝假中性开仓。"""
        logger.warning(f"⛔ 资金费率套利暂未启用：{symbol} 当前没有同时建立永续反向腿")
        return False
    ...
    async def _close_delta_neutral(self, symbol):
        try:
            ...
            if order:
                avg_price = float(order.get('average') or pos['price'])
                filled = float(order.get('filled') or pos['amount'])
                revenue = filled * avg_price
                cost = float(pos['amount_usdt'])
                fee = float(order.get('_fee_cost') or 0)
                pnl = revenue - cost - fee - float(pos.get('entry_fee', 0))
                await self._refresh_balance_cache(force=True)
            ...
            try:
                ...
                for sym in self.symbols:
                    try:
                        ...
                        p = ticker['last']
                        coin = sym.split('/')[0]
                        coin_value = self._bot_position_cost(sym)
                        count = self.position_counts.get(sym, 0)
                        ...
                        if grid:
                            ...
                            if drop_from_last >= grid["drop_pct"]:
                                count = self.position_counts.get(sym, 0)
                                coin_amount = min(float(grid.get("max_order_usdt", self.max_per_coin_usdt or grid["base_amount"] * (1 + self.max_positions_per_coin * grid["increment"]))), float(grid["base_amount"]) * (1 + count * float(grid.get("increment", 0))))
                ...
                for item in candidates:
                    ...
                    if self.max_per_coin_usdt > 0:
                        remaining_coin_budget = self.max_per_coin_usdt - self._bot_position_cost(sym)
                        if remaining_coin_budget <= 0:
                            continue
                        coin_amount = min(coin_amount, remaining_coin_budget)
                    if not await self._can_allocate(coin_amount):
                        logger.info(f'⛔ {sym} 超过总资金占用上限，跳过')
                        continue
                    ...
                    if order:
                        ...
                        net_amount = filled - fee if fee_currency == base_currency else filled
                        real_cost = filled * avg + (fee if fee_currency in ('', 'USDT') else 0)
                        if net_amount <= 0 or real_cost <= 0:
                            self._is_paused = True
                            await self._alert(f'{sym} 买单成交信息异常，已暂停', 'critical')
                            continue
                        ...
                        await save_trade_detail({
                            ...
                            'price': avg, 'amount': filled, 'signal_score': sc, 'fear_greed': fg or 0,
                            'funding_rate': funding or 0, 'pnl_pct': 0, 'real_cost': round(real_cost, 8),
                        ...
                        if settings.TG_CHAT_ID:
                            try:
                                await self.tg_app.bot.send_message(
                                    chat_id=settings.TG_CHAT_ID,
                                    text=f"🤖 开仓 {sym} {coin_amount:.2f}U @ {avg:.4f} 仓位{self.position_counts[sym]}/{self.max_positions_per_coin} | 评分{sc:.0f}"
                                )
                            except:
                                pass
                    else:
            ...
            try:
                ...
                for sym in self.symbols:
                    try:
                        ...
                        entry_price = self._weighted_entry(sym) or self.entries.get(sym, p)
                        ...
                        if action == 'sell':
                            ...
                            if rounded_amount <= 0:
                                continue
                            sell_order = await self.exchange.create_market_sell_order(sym, rounded_amount)
                            if not sell_order:
                                continue
                            ...
                            sell_fee_currency = sell_order.get('_fee_currency', '')
                            quote_currency = sym.split('/')[1]
                            sell_fee_for_pnl = sell_fee if sell_fee_currency in ('', quote_currency) else 0.0
                            try:
                                net_pnl, real_cost, _buy_fee = self._consume_position_lots(sym, sell_filled, sell_avg, sell_revenue, sell_fee_for_pnl)
                            except ValueError as e:
                                logger.error(f"🚨 {sym} 账本与交易所成交不一致: {e}")
                                self._is_paused = True
                                await self._alert(f"{sym} 持仓账本不一致，已暂停自动交易", "critical")
                                continue
                            pnl_pct = ((sell_avg - entry_price) / entry_price) * 100 if entry_price else 0.0
                            real_cost = real_cost if real_cost > 0 else detail.get('real_cost', 0)
                            net_pnl_pct = (net_pnl / real_cost * 100) if real_cost > 0 else pnl_pct
                            ...
                            await save_trade_detail({"time": datetime.now(CST).strftime("%m-%d %H:%M"), "symbol": sym, "side": "sell", "price": sell_avg, "amount": sell_filled, "pnl_pct": round(pnl_pct, 2), "signal_score": detail.get('signal_score', 0), "fear_greed": detail.get('fear_greed', 0), "funding_rate": detail.get('funding_rate', 0), "real_revenue": round(sell_revenue, 8), "fee": sell_fee, "order_id": sell_order.get('id',''), "net_pnl_pct": round(net_pnl_pct, 2)})
                            self._trailing_active[sym] = False
                            self._trailing_high[sym] = 0
                            await self._save_runtime_state()
                            await self._auto_optimize_params()
