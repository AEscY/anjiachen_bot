...
export async function routeUpdate(env, update) {
  ...
  if (update.message) {
    ...
    // 文本指令路由
    if (text === '/start' || text === '/menu') return await handleMenuPage(env);
    if (text === '/status') return await handleQuickStatus(env);
...
async function routePage(env, page) {
  switch (page) {
    case 'main':
      return await handleMenuPage(env);
    case 'grid_panel':
      return await handleGridPanel(env);
    case 'grid_params':
      return await handleGridParams(env);
    case 'grid_fills':
      return await handleGridFills(env, 1);
    case 'dip_panel':
      return await handleDipPanel(env);
    case 'dip_params':
      return await handleDipParams(env);
    case 'dip_history':
      return await handleDipHistory(env, 1);
    case 'settings':
      return await handleSettings(env);
    case 'switch_mode':
      return await handleSwitchMode(env);
    case 'notify_settings':
      return await handleNotifySettings(env);
    case 'help':
      return await handleHelp(env);
    case 'profit':
      return await handleProfitPanel(env);
  }
}
...
async function handleParamInput(env, text, session) {
  ...
  if (target.startsWith('grid_')) {
    ...
    const keyMap = {
      grid_lower: 'lowerPrice',
      grid_upper: 'upperPrice',
      grid_count: 'gridCount',
      grid_amount: 'perGridAmount',
    };
    if (keyMap[target]) {
    ...
    const keyMap = {
      dip_buy: 'targetBuyPrice',
      dip_sell: 'targetSellPrice',
      dip_amount: 'amount',
    };
    if (keyMap[target]) {
      config[keyMap[target]] = value;
  ...
  } else if (target === 'instId') {
    const parts = text.split('/');
    if (parts.length === 2 && parts[0].length <= 6 && parts[1].length <= 6) {
      const instId = parts.join('-');
      const gridConfig = await getState(env, 'GRID_CONFIG');
      gridConfig.instId = instId;
      await setState(env, 'GRID_CONFIG', gridConfig);
      const dipConfig = await getState(env, 'DIP_CONFIG');
      dipConfig.instId = instId;
      await setState(env, 'DIP_CONFIG', dipConfig);
      await sendTelegram(env, `✅ 交易对已切换为 ${instId}，重启策略后生效`);
    } else {
      await sendTelegram(env, '❌ 格式错误，请用 "BTC/USDT" 格式输入');
    }
  }
