...
DEFAULT_CONFIG = {
    ...
    "grid_configs": "{}", "coin_configs": "{}", "max_drawdown_pct": 0.15,
}
            ...
            async with db.execute("SELECT key, value FROM config") as cursor:
                async for key, value in cursor:
                    try:
                        ...
                        logger.warning(f"忽略无效配置: {key}={value!r}")
...
async def load_trades(limit=50):
    ...
    try:
        async with aiosqlite.connect(DB_FILE, timeout=30.0) as db:
            async with db.execute("SELECT time,symbol,entry,exit,pnl_pct,net_pnl,net_pnl_pct FROM trades ORDER BY id DESC LIMIT ?", (limit,)) as cursor:
                async for r in cursor:
                    trades.append({"time":r[0],"symbol":r[1],"entry":r[2],"exit":r[3],"pnl_pct":r[4],"net_pnl":r[5],"net_pnl_pct":r[6]})
...
async def get_recent_performance(num=50):
    try:
        async with aiosqlite.connect(DB_FILE, timeout=30.0) as db:
            async with db.execute("SELECT net_pnl_pct FROM trade_details WHERE side='sell' ORDER BY id DESC LIMIT ?", (num,)) as cursor: rows = await cursor.fetchall()
...
async def get_today_trades():
    ...
    try:
        async with aiosqlite.connect(DB_FILE, timeout=30.0) as db:
            async with db.execute("SELECT net_pnl_pct FROM trade_details WHERE side='sell' AND strftime('%m-%d',time)=? ORDER BY id DESC", (today,)) as cursor: rows=await cursor.fetchall()
        pnls=[r[0] for r in rows if r[0] is not None]
        if not pnls: return None
        wins=sum(p>0 for p in pnls); total=len(pnls)
...
async def save_runtime_state(state: dict):
    """Persist a state snapshot atomically so a crash cannot leave half a snapshot."""
    try:
        async with aiosqlite.connect(DB_FILE, timeout=30.0) as db:
            await db.execute("BEGIN IMMEDIATE")
...
async def get_total_fees():
    """Return only exchange-reported fees stored on fills; never invent a fee rate."""
    try:
        async with aiosqlite.connect(DB_FILE, timeout=30.0) as db:
            async with db.execute("SELECT SUM(fee) FROM trade_details") as cursor: row=await cursor.fetchone()
...
async def get_total_net_profit():
    """Return closed-trade PnL recorded by the execution layer, not account-balance deltas."""
    try:
        async with aiosqlite.connect(DB_FILE, timeout=30.0) as db:
            async with db.execute("SELECT SUM(net_pnl) FROM trades WHERE net_pnl IS NOT NULL") as cursor: row=await cursor.fetchone()
        return round(float(row[0] or 0.0),8) if row else 0.0
